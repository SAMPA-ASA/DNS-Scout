...
try_apt_install_python_packages() {
  ...
  if [[ -n "${py_series}" ]]; then
    ...
    if ${SUDO} apt-get install -y python3 python3-pip "python${py_series}-venv"; then
      return 0
    fi
  fi

  echo "Trying python3-full fallback package."
  if ${SUDO} apt-get install -y python3 python3-pip python3-full; then
    return 0
  fi
...
if ! command -v "${PYTHON_BIN}" >/dev/null 2>&1; then
  echo "Python 3 is not installed. Trying to install it automatically."
fi
...
echo "==> Configuring firewall access for panel port"
ensure_panel_firewall_rule "${SELECTED_PORT}"
...
echo
echo "To print panel login URLs later via CLI:"
echo "  sudo ${INSTALL_DIR}/.venv/bin/python ${INSTALL_DIR}/manage_panel_auth.py --config ${PANEL_CONFIG_FILE} --show-urls"
