...
$branch = if ($env:REPO_BRANCH) { $env:REPO_BRANCH } else { Resolve-DefaultBranchWithFallback -RepositoryUrl $RepoUrl }
...
try {
    ...
    Write-Host "==> Cloning latest source"
    ...
    if (-not (Test-Path $installer)) {
        throw "install.ps1 not found in cloned repository."
    }

    Write-Host "==> Running project installer"
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $installer

    $remoteCommit = (git -C $repoDir rev-parse HEAD).Trim()
    ...
    Write-Host ""
    Write-Host "Online installation completed."
