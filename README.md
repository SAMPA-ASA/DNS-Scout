...
- [پنل](#پنل)
	- [اسکن IPها](#اسکن-ipها)
	- [تست DNSهای یافت‌شده](#تست-dnsهای-یافت%E2%80%8Cشده)
	- [منابع](#منابع)
- [Uninstall](#uninstall)
...
## شروع
توجه داشته باشید که در مراحل نصب، از mirrorهای داخلی و ایرانی، به صورت fallback استفاده شده تا در صورتی که نصب کننده، نتوانست موارد مورد نیاز خود را از منابع اصلی دانلود کند، از منابع ایرانی دانلود کند. بنابراین، توصیه میشود که در طول فرآیند نصب، کل سیستم خودتون رو به VPN متصل نکنید و یا از split tunneling استفاده کنید.
### Ubuntu
#### روش آنلاین
...
- برخی از وابستگی‌ها ممکن است در صورت عدم دسترسی به منابع اصلی، از mirrorهای داخلی دریافت شوند. این mirrorها صرفاً برای افزایش دسترس‌پذیری و پایداری فرآیند نصب در نظر گرفته شده‌اند و توسعه‌دهنده پروژه هیچ تضمینی دربارهٔ دسترس‌پذیری، صحت، امنیت یا به‌روز بودن آن‌ها ارائه نمی‌کند.
